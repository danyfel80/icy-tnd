...
public class SHGAnalysis extends EzPlug implements Block{
	...
	{		
		int w = 	seqIn.getWidth(); int h =  seqIn.getHeight();
		...
		List<ConnectedComponent> ccs = ConnectedComponents.extractConnectedComponents(seqIn, minVolume, maxVolume, null).get(0);
		Point3i[] pts=null;
		int id = 0;
	...
	{		
		...
		/*SomeImageTools.binarize(input2.getValue().getFirstImage(), bin, 0, 0, 1, bt);
		bin.invert();
		List<My2DConnectedComponent> keep = SomeImageTools.findConnectedComponents(bin);
		System.out.println(" Nb cc :"+keep.size());
	...
	private IcyBufferedImage readCortex(Sequence seqBiopsy){
		...
		else imgCortex=seqBiopsy.getFirstImage();
		//}
		//	seqCortex.setName("Cortex");

		/*seqCaps.addImage(imgCortex);
			addSequence(seqCaps);
			seqCaps=null;*/
		return(imgCortex);
	}
	...
	//TODO verfie des type suppose image binaire
	IcyBufferedImage andIcyBufferedImage(IcyBufferedImage img, IcyBufferedImage img2 )
	{
		...
		for (int x = 0; x<w; ++x)
			for (int y = 0; y<h; ++y){
				double val = tabInDouble[x+y*w]; double val2 =tabInDouble2[x+y*w]; 
				if (val>0 && val2>0)
					tabOutDouble[x+y*w]= val;
				else tabOutDouble[x+y*w]= 0;
			}
	...
	IcyBufferedImage notIcyBufferedImage(IcyBufferedImage img)
	{
		...
		double[] tabInDouble = Array1DUtil.arrayToDoubleArray(img.getDataXY(0), img.isSignedDataType());
		double[] minmax = img.getComponentAbsBounds(0);
	...
	private void doQuantifBin(Sequence seqIn, Sequence seqBiopsy){
		...
		System.out.println(" spixels: "+nbsignal+ " imagesize:"+w*h+ " percent:"+(double)nbsignal/(w*h)+" Size biopsy: "+sizeBiopsy+ " percent:"+(double)nbsignal/sizeBiopsy );
	...
	private void doQuantifMean(Sequence seqIn, Sequence seqBiopsy){
		...
		System.out.println(" spixels: "+sizeBiopsy+ " imagesize:"+w*h+ " percent:"+(double)signal/(w*h)+ " percent:"+(double)signal/sizeBiopsy );
	...
	private void doHysteresis(Sequence seqIn){
		...
		double[] minmax = seqIn.getComponentAbsBounds(0);
		double min = minmax[0], max = minmax[1];
		...
		/*	
		double hl = valueLowThreshold.getValue()/256.; // suppose que lq valeur est [0-255]
		double hh = valueHighThreshold.getValue()/256.;

		 */	IcyBufferedImage ima=seqIn.getFirstImage().convertToType(DataType.DOUBLE, true);


		 IcyBufferedImage imaHyst = hysteresisThreshold(ima, hl ,hh );

		 imaHyst = imaHyst.convertToType(DataType.UBYTE, true);
		 Sequence seqOut = new Sequence();
	...
	private void doHysteresis2(Sequence seqIn){
		...
		for (int z = 0; z<seqIn.getSizeZ(); z++) {
			IcyBufferedImage ima=seqIn.getImage(t,z).convertToType(DataType.DOUBLE, true);
			IcyBufferedImage imaHyst = hysteresisThreshold(ima, hl ,hh );
	...
	private void doCrop(Sequence seqIn, int sizeCrop){
		...
		//		File newfolder = new File(currentDirectory + "/CROP"+s.substring(0, s.indexOf("(")) ); // 	nom issu de avec parenthese?
		...
		for ( int y=0; y<h; y=y+hcrop)
			for (int x=0; x<w; x=x+wcrop){
				...
				//		String nomImage = newfolder+ "/" + s.substring(0, s.indexOf("(")) + "v" + inc+".tif"; 
	...
	{
		HYSTERESIS("Hysteresis percentage"), HYSTERESIS2("Hysteresis intensity"), QUANTIF("Quantif"), QUANTIF_Mean("Quantif Mean"), FILL_Hole("Fill hole"), THIN(" thin"), CROP(" crop");
	...
	private void saveResult2Excel(Sequence seq, int sizeBiopsy, double nbsignal, int sizeRemoveSmall, int holeSize){
		...
		/*	int ik=0;
					for (int i=0; i<nbCells; i++) {
						...
						//			 here 0 is the number of the page.
						WritableSheet page = pageresultat.createSheet("cell "+(i+1), i+1 );
						...
						label = new Label( col+3 , 0 , "Fibrosis:" );
						page.addCell(label);
						label = new Label( col+4 , 0 , " :" );
						page.addCell(label);
						int li=1; // indice line
						int nbSpotsC = cNbSpots.get(i);
						double sIntensity=0.;
						int nbCellLim = ik+nbSpotsC;
						int sizeSpotbyCell = 0; // taille des spots dans une cellule
						for (int k=ik ;k<nbCellLim; k++) {
							Spot obj =null;
							//	if (k<clistSpot.size()) //{
							obj = (Spot) clistSpot.get(k);
							int x = (int) Math.rint(obj.mass_center.x);
							int y = (int) Math.rint(obj.mass_center.y);
							int z = (int) Math.rint(obj.mass_center.z);

							col=0;
							ik++;
							Number number = new Number( col , li , x );
							page.addCell(number);
							number = new Number( col+1 , li , y );
							page.addCell(number);
							number = new Number( col+2 , li , obj.point3DList.size() );
							page.addCell(number);
							number = new Number( col+3 , li , obj.meanIntensity );
							page.addCell(number);
							number = new Number( col+4 , li , obj.meanIntensity*obj.point3DList.size() );
							page.addCell(number);
							sIntensity += obj.meanIntensity*obj.point3DList.size();

							sizeSpotbyCell+=obj.point3DList.size();
							//	}
							li++;
					...
					int li =1;
					for (int k=0 ;k<nbCells; k++) {//TODO limit
						col=0;
						Number	number = new Number( col , li , (k+1) );
						page.addCell(number);
						col++; 
	...
	public double computeDistancePointfromSegment(Point3d pt, Point3d ptA, Point3d ptB){
		...
		/*	Point3d ptH = new Point3d();
		ptH.x = ptA.x + distAHcarre/prodScalaire*(ptB.x-ptA.x);
		ptH.y = ptA.y + distAHcarre/prodScalaire*(ptB.y-ptA.y);*/
	...
	private void computeUnshading( File fileIn, Sequence seqShading)
	{
		System.gc();
		...
		if ( !fileIn.getAbsolutePath().endsWith(".tif") )
		{
			System.out.println("Ignoring file " + fileIn );
			return;
		}
		...
		Sequence sequence = new Sequence();
		Loader loader = new Loader( sequence , new File[]{fileIn});		
		loader.start();
		try {
			loader.join();
		...
		double [] input = ImageDoubleConverter.convertFrom(sequence.getImageAt(0))[0];
		double [] bufShading = ImageDoubleConverter.convertFrom(seqShading.getImageAt(0))[0];
		double [] bufOutput = new ImageDouble(input.width,input.height,input.depth,input.resolutionX,input.resolutionY,input.resolutionZ);

		for (int i=0; i<input.width*input.height; i++)
			bufOutput.data[i]=input.data[i]/bufShading.data[i]*correctValue; // TODO: Verifier: correctValue ???

		 currentdir = fileIn.getAbsolutePath();
		 currentdir = currentdir.substring( 0 , currentdir.length() - sequence.file[0].getName().length() );
		...
		if (nbf<10)
			 filename = currentdir + "RESULT/" + "StackCorr_000"+nbf; 
			else if (nbf<100)
				filename = currentdir + "RESULT/" + "StackCorr_00"+nbf; 
			else if (nbf<1000)
				filename = currentdir + "RESULT/" + "StackCorr_0"+nbf; 
			else
			filename = currentdir + "RESULT/" + "StackCorr_"+nbf; 
	//	filename = "/Users/vannary0/Desktop/SHG2/pgmRecalagePourVannary/patient1/05_2333_06/testRedress"; 
		System.out.println("  filename : "+filename);
		nbf++;
		Saver saver = new Saver( seqOut, new File(  filename), FileType.TIFF, false);
		saver.run();
