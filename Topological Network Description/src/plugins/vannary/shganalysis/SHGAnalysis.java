...
public class SHGAnalysis extends EzPlug implements Block {
  ...
  private EzVarBoolean  inputShowResults = new EzVarBoolean("Show Result",
      true);
  ...
  protected void initialize() {
    ...
    EzGroup hysteresisVar = new EzGroup("Hysteresis parameters",
        valueLowThreshold, valueHighThreshold);
    super.addEzComponent(hysteresisVar);

    method.addVarChangeListener(new EzVarListener<MethodType>() {
      @Override
      public void variableChanged(EzVar<MethodType> source,
          MethodType newValue) {
        updateDefaultParams();
      }
  ...
  private Sequence remove_by_size(Sequence seqIn, int size) {
    ...
    double[] tabIn = Array1DUtil.arrayToDoubleArray(img.getDataXY(0),
        img.isSignedDataType());
    List<ConnectedComponent> ccs = ConnectedComponents
        .extractConnectedComponents(seqIn, minVolume, maxVolume, null).get(0);
    Point3i[] pts = null;
    int id = 0;
    ...
    imgOut.setDataXY(0,
        Array1DUtil.doubleArrayToArray(tabIn, imgOut.getDataXY(0)));
  ...
  private IcyBufferedImage readCortex(Sequence seqBiopsy) {
    ...
    currentdir = currentdir.substring(0,
        currentdir.length() - (seqBiopsy.getName().length() + 4));
  ...
  IcyBufferedImage andIcyBufferedImage(IcyBufferedImage img,
      IcyBufferedImage img2) {

    int w = img.getWidth();
    int h = img.getHeight();

    double[] tabInDouble = Array1DUtil.arrayToDoubleArray(img.getDataXY(0),
        img.isSignedDataType());
    double[] tabInDouble2 = Array1DUtil.arrayToDoubleArray(img2.getDataXY(0),
    ...
    img.setDataXY(0,
        Array1DUtil.doubleArrayToArray(tabOutDouble, img.getDataXY(0)));
  ...
  IcyBufferedImage notIcyBufferedImage(IcyBufferedImage img) {
    ...
    img.setDataXY(0,
        Array1DUtil.doubleArrayToArray(tabInDouble, img.getDataXY(0)));
  ...
  private void doQuantifBin(Sequence seqIn, Sequence seqBiopsy) {
    ...
    double[] tabInDouble = Array1DUtil.arrayToDoubleArray(imaIn.getDataXY(0),
        imaIn.isSignedDataType());

    double[] tabCortexDouble = Array1DUtil.arrayToDoubleArray(
        imgCortex.getDataXY(0), imgCortex.isSignedDataType());
    ...
    System.out.println(" spixels: " + nbsignal + " imagesize:" + w * h
        + " percent:" + (double) nbsignal / (w * h) + " Size biopsy: "
        + sizeBiopsy + " percent:" + (double) nbsignal / sizeBiopsy);
  ...
  private void doQuantifMean(Sequence seqIn, Sequence seqBiopsy) {
    ...
    System.out.println(" spixels: " + sizeBiopsy + " imagesize:" + w * h
        + " percent:" + (double) signal / (w * h) + " percent:"
        + (double) signal / sizeBiopsy);
  ...
  private void doHysteresis(Sequence seqIn) {
    ...
    IcyBufferedImage ima = IcyBufferedImageUtil
        .convertToType(seqIn.getFirstImage(), DataType.DOUBLE, true);
  ...
  private void doCrop(Sequence seqIn, int sizeCrop) {
    ...
    for (int y = 0; y < h; y = y + hcrop)
      for (int x = 0; x < w; x = x + wcrop) {
        ...
        IcyBufferedImage crop = IcyBufferedImageUtil.getSubImage(buf, x, y,
            wcrop, hcrop);
  ...
  private void doImageFromCrops(Sequence seqIn, int sizeCrop) {
    ...
    File newfolder = new File(
        currentDirectory + "/CROP" + s.substring(0, s.indexOf("(")));
    newfolder.mkdir();
    int inc = 1;
  ...
  private enum MethodType {
    HYSTERESIS("Hysteresis percentage"), HYSTERESIS2(
        "Hysteresis intensity"), QUANTIF("Quantif"), QUANTIF_Mean(
            "Quantif Mean"), FILL_Hole(
                "Fill hole"), THIN(" thin"), CROP(" crop");
  ...
  private void saveResult2Excel(Sequence seq, int sizeBiopsy, double nbsignal,
      int sizeRemoveSmall, int holeSize) {
    ...
    file = file.substring(0,
        file.length() - inputSequenceFile.getName().length());
    String folder = file;
    ...
    String nameFile = folder + "SAVE" + File.separator + filename + "_"
        + sizeRemoveSmall + "_" + holeSize + ".xls";
  ...
  public static IcyBufferedImage hysteresisThreshold(IcyBufferedImage img,
      double hl, double hh) {
  ...
  public double computeDistancePointfromSegment(Point3d pt, Point3d ptA,
      Point3d ptB) {
    double res = 0;
    double prodScalaire = 0;

    prodScalaire = (pt.x - ptA.x) * (ptB.x - ptA.x)
        + (pt.y - ptA.y) * (ptB.y - ptA.y);
