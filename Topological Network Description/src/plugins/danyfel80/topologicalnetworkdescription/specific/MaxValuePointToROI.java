	...
	protected void execute() {
		...
		if (inSequence.getValue() != null && !inSequence.getValue().isEmpty()) {
			...
			if (inSequence.getValue().getSizeC() > 1) {
				MessageDialog.showDialog("Warning... Multi-channel sequence",
				    "The input image has more than one channel. Using only the first channel.", MessageDialog.WARNING_MESSAGE);
			}
