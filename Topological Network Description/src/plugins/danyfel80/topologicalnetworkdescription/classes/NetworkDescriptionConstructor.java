...
public class NetworkDescriptionConstructor {
	...
	public Sequence process() {
		...
		for (int z = 0; z < sizeZ; z++) {
			...
			IcyBufferedImage tempEImage = new IcyBufferedImage(sizeX, sizeY, 1, DataType.BYTE);
			IcyBufferedImage tempYImage = new IcyBufferedImage(sizeX, sizeY, 1, DataType.BYTE);
			IcyBufferedImage tempSImage = new IcyBufferedImage(sizeX, sizeY, 1, DataType.BYTE);
		...
		while (!q.isEmpty()) {
			...
			if (labelData[ceZ][0][ceX+ceY*sizeX] == 0) {
				...
				int rs = (int) Math.ceil(Math.sqrt(distanceData[ceZ][0][ceX+ceY*sizeX])*2.5);
				rs*=rs;
				...
				endPointData[ceZ][0][ceX+ceY*sizeX] = 1;
				skeletonData[ceZ][0][ceX+ceY*sizeX] = 1;
				...
				do {
					...
					if (addNewPoints) {
						if (skeletonData[ceZ][0][ceX+ceY*sizeX] == 0) {
							labelData[ceZ][0][ceX+ceY*sizeX] = branchId;
							// mark neighbor sphere
							rs = (int) Math.ceil(Math.sqrt(distanceData[ceZ][0][ceX+ceY*sizeX])*2.0);
							rs*=rs;
							r = (int) Math.ceil(Math.sqrt(rs));
							for (int x = 0; x <= r; x++) {
								for (int y = 0; y <= r; y++) {
									for (int z = 0; z <= r; z++) {
										if (x*x+y*y+z*z <= rs) {
							...
							branchData[ceZ][0][ceX+ceY*sizeX] = 1;
							branchId++;
							addNewPoints = false;
						}
					}
					else {
						if (branchData[ceZ][0][ceX+ceY*sizeX] == 0) {
							labelData[ceZ][0][ceX+ceY*sizeX] = branchId;
							// mark neighbor sphere
							rs = (int) Math.ceil(Math.sqrt(distanceData[ceZ][0][ceX+ceY*sizeX])*2.0);
							rs*=rs;
			...
			if (labeledSkeletonSequence == null) {
				...
				for (int z = 0; z < sizeZ; z++) {
					...
					for (int xy = 0; xy < sizeX*sizeY; xy++) {
						tempData[0][xy] = (skeletonData[z][0][xy] == 0? 0: labelData[z][0][xy]);
					}
	...
	public DirectedGraph<Point3i, DefaultEdge> getGraph() {
		...
		for (int z = 0; z < sizeZ; z++) {
			for (int x = 0; x < sizeX; x++) {
				for (int y = 0; y < sizeY; y++) {
					if (endPointData[z][0][x+y*sizeX] > 0) {
						...
						while (!p.equals(pp)) {
							if (branchData[pp.z][0][pp.x + pp.y*sizeX] > 0) {
								graph.addVertex(pp);
								graph.addVertex(b);
								graph.addEdge(pp, b);
								b = pp;
