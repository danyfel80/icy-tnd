...
public class CostToSeedMapGenerationPlugin extends EzPlug implements Block {
	...
	protected void execute() {
		...
		if (originalSequence.getROIs(ROI2DPoint.class, false).size() == 0) {
		...
		List<ROI2DPoint> seeds = originalSequence.getROIs(ROI2DPoint.class, false);
		...
		CostToSeedMapGenerator ctsc = new CostToSeedMapGenerator(invertedDistanceMapSequence, seeds, levelChangeWeightMultiplier, directionChangeWeightMultiplier);
