  ...
  public static Sequence process(Sequence thresholdedSequence, List<ROI2DPoint> seeds) {
    ...
    for (int z = 0; z < thresholdedSequence.getSizeZ(); z++) {
      IcyBufferedImage tempImage = new IcyBufferedImage(thresholdedSequence.getSizeX(),
          thresholdedSequence.getSizeY(), 1, DataType.UBYTE);
    ...
    while (!q.isEmpty()) {
      ...
      if (thresholdedData[pZ][0][pX + (pY) * sizeX] > 0 &&
          TypeUtil.unsign(resultData[pZ][0][pX + (pY) * sizeX]) == 0) {
        resultData[pZ][0][pX + (pY) * sizeX] = (byte)DataType.UBYTE.getMaxValue();

        // <x-1, y, z>
        if (pX > 0 &&
            TypeUtil.unsign(resultData[pZ][0][pX-1 + pY*sizeX]) == 0)
          q.add(new Point5D.Double(pX-1, pY, pZ, pT, pC));
        // <x, y-1, z>
        if (pY > 0  &&
            TypeUtil.unsign(resultData[pZ][0][pX + (pY-1)*sizeX]) == 0)
        ...
        // <x, y, z-1>
        if (pZ > 0 &&
            TypeUtil.unsign(resultData[pZ-1][0][pX + pY*sizeX]) == 0)
          q.add(new Point5D.Double(pX, pY, pZ-1, pT, pC));
        if (pZ < sizeZ-1 &&
            TypeUtil.unsign(resultData[pZ+1][0][pX + pX*sizeX]) == 0)
          q.add(new Point5D.Double(pX, pY, pZ+1, pT, pC));
      }
    }
    result.dataChanged();
